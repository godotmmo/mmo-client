...
var player_spawn: Resource = preload("res://scripts/entity_scripts/Player/PlayerTemplate.tscn")
...
func _physics_process(_delta: float) -> void:
	var render_time: int = Server.client_clock_msecs - interpolation_offset
		...
		if world_state_buffer.size() > 2: # We have a future world state | Interpolate
			...
			for player in world_state_buffer[2].keys():
				...
				if get_node("../Map/OtherPlayers").has_node(str(player)):
					...
					var new_position: Vector3 = world_state_buffer[1][player]["P"].lerp(world_state_buffer[2][player]["P"], clamp(interpolation_factor, 0, 1))
			...
			var extrapolation_factor: float = float(render_time - world_state_buffer[0]["T"]) / clamp(float(world_state_buffer[1]["T"] - world_state_buffer[0]["T"]), 0.0001, 100000) - 1.00
...
func SpawnNewPlayer(player_id: int, spawn_position: Vector3) -> void:
	...
	if multiplayer.get_unique_id() != player_id and not get_node("../Map/OtherPlayers").has_node(str(player_id)):
		...
		var new_player: Node = player_spawn.instantiate()
...
func DespawnPlayer(player_id: String) -> void:
